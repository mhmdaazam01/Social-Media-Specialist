...
export function AppShell({ children, title = 'Dashboard', onAddPost }: AppShellProps) {
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-cly-bg print:h-auto print:block print:bg-white">
      ...
      <div className="flex-1 flex flex-col min-w-0 lg:ml-[280px] pb-16 lg:pb-0 bg-cly-bg print:ml-0 print:pb-0 print:bg-white print:block">
        <div className="print:hidden"><Topbar title={title} onAddPost={onAddPost} /></div>
        <main className="flex-1 overflow-y-auto overflow-x-hidden min-w-0 p-3 sm:p-4 md:p-6 lg:p-8 bg-cly-bg print:overflow-visible print:p-0 print:bg-white print:block">
          {children}
        </main>
      </div>
